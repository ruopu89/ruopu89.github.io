...
//访问安装pyenv，安装好后会输入三行信息，要加入.bash_profile文件中。使用-L参数，curl就会跳转到新的网址。
vim ~/.bash_profile
...
mkdir cache
//创建一个目录，下面准备使用离线安装的方式。
cd cache
将python-3.5.3.tar.gz、python-3.5.3.tar.xz、python-3.5.3.tgz三个文件放入cache目录中。这三个文件是在安装过程中可能依赖的包
cd ..
pyenv install 3.5.3 -v
//在cache上一层目录执行安装
python -V
...
pip list
//现在包很少
pip install -r ../test/reguiremeot
